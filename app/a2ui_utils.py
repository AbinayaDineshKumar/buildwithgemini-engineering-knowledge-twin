...
def _ensure_begin_rendering(messages: list[dict]) -> None:
    """Ensure every surfaceUpdate has a corresponding beginRendering message.

    If the model emitted a surfaceUpdate without a beginRendering, synthesize
    one by finding the root component (an unreferenced component or top component).
    """
    existing_surfaces = set()
    for m in messages:
        if "beginRendering" in m and isinstance(m["beginRendering"], dict):
            sid = m["beginRendering"].get("surfaceId")
            if sid:
                existing_surfaces.add(sid)

    new_messages = []
    for m in messages:
        if "surfaceUpdate" in m and isinstance(m["surfaceUpdate"], dict):
            su = m["surfaceUpdate"]
            sid = su.get("surfaceId", "main_surface")
            if sid not in existing_surfaces:
                comps = su.get("components") or []
                if comps:
                    ids, refs = _component_ids_and_refs(comps)
                    unref = ids - refs
                    root_id = None
                    for candidate in ("root", "root_card", "root-card", "main", "card"):
                        if candidate in unref:
                            root_id = candidate
                            break
                    if not root_id and unref:
                        root_id = sorted(list(unref))[0]
                    if not root_id and comps and isinstance(comps[0], dict) and "id" in comps[0]:
                        root_id = comps[0]["id"]
                    if root_id:
                        new_messages.append({"beginRendering": {"surfaceId": sid, "root": root_id}})
                        existing_surfaces.add(sid)
